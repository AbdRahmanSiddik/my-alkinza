<div>
    <div class="row pos-wrapper m-2">
                                                ...
                                                <div class="d-flex flex-column gap-2">
                                                    @if ($transaction->status == 'selesai')
                                                        ...
                                                        <button class="btn btn-sm btn-danger" type="button"
                                                            wire:click="cancelTransaction('{{ $transaction->kode_transaksi }}')">Ajukan
                                                            Pembatalan</button>
                                                    @else
                                                        <button class="btn btn-sm btn-warning" type="button"
                                                            disabled>Pembatalan Pending</button>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        <div class="border-top pt-3">
                            ...
                            @if ($detail?->metode_pembayaran == 'tunai')
                                <div class="d-flex justify-content-between mb-1">
                                    ...
                                    <span>@rupiah($detail->bayar ?? 0)</span>
                                </div>
                                <div class="d-flex justify-content-between mb-1">
                                    <small class="text-muted">Kembali</small>
                                    <span>@rupiah($detail->kembali ?? 0)</span>
                                </div>
                            @endif
        ...
        <div class="col-md-12 col-lg-5 col-xl-4 ps-0">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3>Cash Flow</h3>
                    <div>
                        <button wire:click="termalPrint" class="btn btn-secondary" {{ $cantPrint ? 'disabled' : '' }}>Print Rekap</button>
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kas-modal">Tambah
                            Kas</button>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    @forelse($kas as $item)
                        <div class="alert alert-{{ $item->jenis == 'masuk' ? 'success' : 'danger' }} mb-2"
                            role="alert">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <strong>{{ ucfirst($item->jenis) }}</strong>
                                    <div class="small text-muted mt-1">
                                        {{ $item->created_at->translatedFormat('d-m-Y H:i') }}</div>
                                    @if ($item->nama)
                                        <div class="small mt-1">{{ $item->nama }}</div>
                                    @endif
                                </div>
                                <div>
                    ...
                    @empty
                        <div class="alert alert-info" role="alert">
                            <div class="text-center text-muted py-2">Tidak ada data kas</div>
                        </div>
                    @endforelse
                </div>
            </div>
        ...
        <!-- /Order Details -->
        <div class="modal fade" id="kas-modal" wire:ignore.self>
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form wire:submit.prevent="saveKas">
                            <div class="mb-3">
                                <label for="namaKas" class="form-label">Nama Kas</label>
                                <input type="text" id="namaKas" class="form-control" wire:model="namaKas">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Jenis Kas</label>
                                <div class="form-group d-flex gap-3">
                                    <div class="form-check">
                                        <input type="radio" id="masuk" class="form-check-input"
                                            wire:model="jenis" value="masuk">
                                        <label class="form-check-label" for="masuk">Kas Masuk</label>
                                    </div>
                                    <div class="form-check">
                                        <input type="radio" id="keluar" class="form-check-input"
                                            wire:model="jenis" value="keluar">
                                        <label class="form-check-label" for="keluar">Kas Keluar</label>
                                    </div>
                                </div>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="keterangan" class="form-control" wire:model="keterangan"></textarea>
                            </div>
